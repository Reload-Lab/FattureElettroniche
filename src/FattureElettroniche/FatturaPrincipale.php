...
class FatturaPrincipale extends Tag {
			...
			if($name = $this->getName($name)){
				...
				if($name == 'NumeroFatturaPrincipale'){
					...
					if(!is_string($value) 
						...
						$this->err()->setErrors(_('Numero Fattura Principale "'.$value.'": Formato alfanumerico; lunghezza massima di 20 caratteri in '.__FILE__.' on line '.__LINE__));
						return;
					}
				}
				...
				if($name == 'DataFatturaPrincipale'){
					...
					if(!preg_match('/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/', $value) 
						...
						$this->err()->setErrors(_('Data Fattura Principale "'.$value.'": La data deve essere rappresentata secondo il formato ISO 8601:2004, con la seguente precisione: YYYY-MM-DD in '.__FILE__.' on line '.__LINE__));
						return;
					}
				}
	...
	public function getXml()
	{
		...
		if($this->NumeroFatturaPrincipale != ''){
			...
			$this->err()->setErrors(_('Numero Fattura Principale: Il tipo è obbligatorio in '.__FILE__.' on line '.__LINE__));
		}
		...
		if($this->DataFatturaPrincipale != ''){
			...
			$this->err()->setErrors(_('Data Fattura Principale: Il tipo è obbligatorio in '.__FILE__.' on line '.__LINE__));
		}
